# GitHub 相似项目对照：原人物分析方向与当前公益安全方向

调研日期：2026-09-24（本轮复核）
范围：本文件有两个时间和产品方向不同的切片：前半部分比较旧版人物/关系分析工具的 8 个仓库；后半部分围绕公益反诈/反操控方向比较 11 个仓库。这里只据仓库公开 README、Skill 文件和可见结构描述设计，不把项目自述等同于实际效果证明。

本文件记录定向选出的 19 个仓库样本，不推断 GitHub 全站规模或总体分布。两个切片对应不同产品定位，不应简单理解为 19 个直接竞品。仓库内容会变化，结论应视为 2026-09-24 附近的查阅快照。

> **后续命名更新：** 本快照末尾关于“防骗与反操控”、`person-analysis` 和旧仓库 slug 的内容记录当时的决策，已被 [后续相邻项目研究与命名评审](interaction-risk-skills-landscape.md)取代。当前中文展示名为“互动风险识别与解构”，Skill ID 与仓库 slug 为 `interaction-risk-analysis`。

## 摘要

当前项目已把主线改为：帮助普通人核对诈骗、心理操控、胁迫与霸凌风险，并让使用者看见判断依据、未知和可选核验步骤。11 个公益安全样本中，未发现一个同时与“消费者可安装 Skill + 跨诈骗/关系操控/霸凌 + 非责备且主体性优先”完整同型的仓库；但诈骗 Skill、操控识别原型、MCP 工具和评测已有相邻尝试。因此只能说定位组合有差异，不能称为唯一或准确率领先。

此前的人物/关系分析方向及其 8 个仓库对照作为历史研究保留，不能当作当前主产品介绍。

前半部分保留原项目方向的历史对照观察；后半部分记录产品转向后的定向样本、竞品边界与当前实现状态。
...
本节记录的是 2026-09-24 时的分发设计。当前唯一运行时包已调整为 `skills/person-analysis/`，通过 `--skill person-analysis` 安装；它把旧人物分析方法与反诈、反操控和安全支持合并，其他研究、案例及校验脚本留在仓库级。当前发现与隔离安装结果记录于本轮验收报告。
...
当前项目名为 **防骗与反操控**，唯一安装 Skill 是 `person-analysis`。它恢复旧版人物分析中的事件拆解、互动模式、竞争解释、证据/反证和按需多视角，并将这套方法优先用于社交工程/电诈、亲密关系操控、职场及社交霸凌。它不诊断人格或疾病，不以“识别坏人”为目标，也不替用户决定是否离开关系或举报。
...
仓库样本的名称大多直接标出问题或功能（Scam Sentinel、Fraud Prevention、TrustAble、clawback、phish-n-cheats），而前文关系类 Skill 名称偏分析/关系顾问。项目若转向公益型防护，名称宜优先让普通用户看懂“防骗 + 识别操控”，再用副标题承载人文关怀；不要把黑暗三联征、煤气灯、霸凌、电诈等全部列进主名称，也不要暗示能识破人格或保证识骗。
...
当前采用“**防骗与反操控**”作为项目中文名，以“**看清套路，自己判断**”说明人物/互动解构的方法。唯一 Skill ID 为 `person-analysis`；GitHub 远端仓库 slug 暂保留 `evidence-based-person-analysis`，本轮没有改动远端地址。
